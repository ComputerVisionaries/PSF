...
def flatten_image(image, piece_size, indexed=False):
    ...
    :params piece_size: Size of single square piece. Each piece is PIECE_SIZE[1] x PIECE_SIZE[0]
    ...
    Usage::
        ...
        >>> flat_image = flatten_image(image, (20, 30))

    """
    rows, columns = image.shape[0] // piece_size[1], image.shape[1] // piece_size[0]
    pieces = []

    x_piece_size, y_piece_size = piece_size
    # Crop pieces from original image
    for y in range(rows):
        for x in range(columns):
            left, top, w, h = x * x_piece_size, y * y_piece_size, (x + 1) * x_piece_size, (y + 1) * y_piece_size
            piece = np.empty((y_piece_size, x_piece_size, image.shape[2]))
            piece[:y_piece_size, :x_piece_size, :] = image[top:h, left:w, :]
            pieces.append(piece)
